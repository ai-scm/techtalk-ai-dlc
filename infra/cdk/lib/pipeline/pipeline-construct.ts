        ...
        phases: {
          ...
          build: {
            commands: [
              'export PATH=$HOME/.garden/bin:$PATH',
              '',
              '# Ensure admin SSO role has cluster access (idempotent)',
              'SSO_ROLE_ARN="arn:aws:iam::891377180652:role/aws-reserved/sso.amazonaws.com/AWSReservedSSO_PS-CloudSeniorEngineerAccess_4afeb42c0f0110a3"',
              'if ! kubectl get configmap aws-auth -n kube-system -o yaml | grep -q "AWSReservedSSO_PS-CloudSeniorEngineerAccess"; then',
              '  echo "Adding admin SSO role to aws-auth..."',
              '  CURRENT=$(kubectl get configmap aws-auth -n kube-system -o jsonpath=\'{.data.mapRoles}\')',
              '  NEW_ENTRY="- rolearn: $SSO_ROLE_ARN\\n  username: admin-sso\\n  groups:\\n  - system:masters"',
              '  kubectl get configmap aws-auth -n kube-system -o json | python3 -c "',
              'import json,sys; cm=json.load(sys.stdin); roles=cm[\"data\"][\"mapRoles\"]; cm[\"data\"][\"mapRoles\"]=roles+\"\\n- rolearn: arn:aws:iam::891377180652:role/aws-reserved/sso.amazonaws.com/AWSReservedSSO_PS-CloudSeniorEngineerAccess_4afeb42c0f0110a3\\n  username: admin-sso\\n  groups:\\n  - system:masters\\n\"; json.dump(cm,sys.stdout)" | kubectl apply -f -',
              'fi',
              '',
